...
export default function AdminSchedule() {
  ...
  const [form, setForm] = useState({
    ...
    job_type: 'setup', status: 'draft', notes: ''
  });
  ...
  const filteredEmps = useMemo(() => {
    ...
    return employees.filter(e => [e.full_name ?? '', e.email ?? '', e.id]
  ...
  async function createShift() {
    ...
    try {
      ...
      const j: any = await parseMaybeJson(r);
      if (!r.ok) throw new Error(j?.error || j?.raw || `HTTP ${r.status}`);
      setForm({ start_time:'', end_time:'', location_name:'', address:'', job_type:'setup', status:'draft', notes:'' });
  ...
  return (
    <div className="page">
      ...
      <div className="card" style={{ padding: 12 }}>
        ...
        <div className="mt-lg">
          <button type="button" className="btn-primary" onClick={createShift} disabled={creating||!form.start_time||!adminId}>
      ...
      <div className="mt-lg">
        ...
        {!loading && upcoming.length === 0 && !err && (
          <div className="card" style={{ padding: 12 }}><div className="muted">No upcoming scheduled shifts.</div></div>
        )}
        {upcoming.length > 0 && (
            ...
            <table className="table table--admin">
              <thead>
                <tr>
                  ...
                  <th>Assigned</th><th>Status</th><th className="col-hide-md">Notes</th><th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {upcoming.map(r => {
      ...
      {edit && (
        <div id="edit-panel" className="card mt-lg" style={{ padding: 16 }}>
          ...
          <label className="mt-lg">Status</label>
          <select value={edit.status ?? 'draft'} onChange={(e)=>setEdit({...edit,status:e.target.value as any})}>
            <option value="draft">Draft</option>
            <option value="confirmed">Confirmed</option>
            <option value="changed">Changed</option>
          </select>
      ...
      {assignShift && (
        <div id="assign-panel" className="card mt-lg" style={{ padding: 16 }}>
          <div className="row between">
            <strong>Assign Employees — {assignShift.location_name || 'Shift'} ({fmt(assignShift.start_time)})</strong>
