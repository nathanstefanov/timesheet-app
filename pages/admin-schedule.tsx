...
/* =========================
   Google Maps loader (no package)
   - loads JS with loading=async
   - supports Places library + functional API
========================= */
...
/* =========================
   LocationPicker (PlaceAutocompleteElement)
========================= */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Optional: restrict to US (remove to allow global)
...
export default function AdminSchedule() {
  ...
  async function createShift() {
    ...
    const v = validateForm(); setFormError(v); if (v) return;

    setCreating(true);
    try {
      const startLocal = combineLocalDateTime(form.start_date, form.start_time);
      const endLocal = form.use_duration
        ? addHoursToLocalInput(startLocal, form.duration_hours).slice(0, 16)
