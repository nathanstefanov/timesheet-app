    ...
    <div className="page">
      ...
      {shiftId && (
        <div className="card mt-lg" style={{ padding: 12 }}>
          ...
          <div className="mt-lg">
            <input className="full" placeholder="Search by name or email…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          ...
          <div className="mt-lg" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
            {filtered.map((e) => (
              <label key={e.id} className="inline-check card" style={{ padding: 10 }}>
                ...
                <span>{e.full_name || e.id.slice(0, 8)}{e.email ? <span className="muted"> • {e.email}</span> : null}</span>
              </label>
            ))}
          </div>

          <div className="mt-lg">
            <button className="btn-primary" onClick={assignEmployees} disabled={assigning || selected.length === 0}>
