...
type ProfileRow = {
  ...
  venmo_handle?: string | null; // e.g. "aidan-stefanov" or "@aidan-stefanov"
  venmo_url?: string | null;    // e.g. "https://venmo.com/u/aidan-stefanov"
  venmo?: string | null;        // optional convenience column (handle)
};
...
/** Build a plain Venmo profile URL from any of our supported columns. */
function venmoHrefFromRow(row?: ProfileRow | null): string | null {
  if (!row) return null;
  // Prefer explicit URL if provided
  if (row.venmo_url && row.venmo_url.trim()) return row.venmo_url.trim();

  // Otherwise use handle from venmo_handle or venmo
  const raw = (row.venmo_handle || row.venmo || '')?.trim();
  if (!raw) return null;
  const handle = raw.replace(/^@/, ''); // strip leading @ if present
...
export default function Admin() {
  ...
  // Simple map of user id -> { name, venmoHref }
  const [people, setPeople] = useState<Record<string, { name: string; venmoHref: string | null }>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Fetch names + venmo details for display
        const ids = Array.from(new Set(rows.map((s: any) => s.user_id)));
        if (ids.length) {
          const { data: profs } = await supabase
            .from('profiles')
            .select('id, full_name, venmo_handle, venmo_url, venmo')
            .in('id', ids);

          const map: Record<string, { name: string; venmoHref: string | null }> = {};
          (profs || []).forEach((p: ProfileRow) => {
            map[p.id] = {
              name: p.full_name || '—',
              venmoHref: venmoHrefFromRow(p),
            };
          });
          setPeople(map);
        } else {
  ...
  async function bulkTogglePaidForEmployee(userId: string, next: boolean) {
    ...
    const name = people[userId]?.name || 'employee';
  ...
  async function deleteRow(row: any) {
    if (!confirm(`Delete shift for ${people[row.user_id]?.name || 'employee'} on ${row.shift_date}?`)) return;
          ...
          <table className="table table--center table--compact table--admin">
            ...
            <tbody>
              {sortedTotals.map((t) => {
                ...
                const venmoHref = person?.venmoHref || null;
                const showVenmo = venmoHref && t.unpaid > 0;
                return (
                  <tr key={t.id}>
                    ...
                    <td data-label="Unpaid">
                      ...
                      {showVenmo && (
                        <a
                          ...
                          title={`Open ${person?.name}'s Venmo`}
                        >
                          Venmo
                        </a>
                      )}
                      ...
                      <tr key={s.id}>
                        ...
                        <td data-label="Actions">
                          <div className="actions">
                            <button className="btn-edit" onClick={() => r.push(`/shift/${s.id}`)}>Edit</button>
